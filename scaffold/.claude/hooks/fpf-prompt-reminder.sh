...
Source code modifications (Write/Edit) are BLOCKED by the PreToolUse hook
until you complete Gate 0:

  1. MUST invoke /fpf-core (this writes the sentinel and unblocks edits)
  2. MUST invoke /fpf-worklog <goal> (this creates the audit trail)
  3. Only then may you begin substantive work

This is a MECHANICAL gate, not advisory. Write/Edit calls outside .fpf/
will be denied with an error until the sentinel exists.

If this is a trivial question (< 3 tool uses, no code changes), you may
answer directly without Gate 0.
REMINDER
