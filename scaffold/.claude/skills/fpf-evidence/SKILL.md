...
If evidence **corroborates** but with caveats:
- Note the scope limitations in G (ClaimScope)
- Set valid_until if the evidence can go stale
...
- **C7:** valid_until set if evidence can go stale (time-dependent claims, version-dependent behavior)
- **C8:** If refuted: note what changes in the problem framing (feedback to PROB-*)
